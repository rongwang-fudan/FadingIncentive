...
if Purpose==1
%Monte Carlo without observational constraints
[ duds ] = MIC( FFlux, L, dpo, dcoef, xy_damage, LR, covidyear, abtfrac, abtlen );
%Plot MIC
plot(duds(1:10,15),'LineStyle','-','LineWidth',3  ,'Color',[0 0 0]); hold on; % rou=0.015
plot(duds(1:10,10),'LineStyle','--','LineWidth',3  ,'Color',[0.2 0.2 0.2]); hold on; % rou=0.015
plot(duds(1:10,25),'LineStyle',':','LineWidth',3  ,'Color',[0.2 0.2 0.2]); hold on; % rou=0.015
end
...
%Monte Carlo with observational constraints
if Purpose==3
load('D:\monte carlo1\uncerts2.dat','-mat');
[para_range2] = MonteCarlo2( FFlux, L, dpo, dcoef, xy_damage, LR, covidyear, abtfrac, abtlen, uncerts2, 0 );
...
[duyy] = plot_dUdt( elasmu );

save('output\duyy.dat','duyy');
end
