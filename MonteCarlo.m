...
function [ para_range ] = MonteCarlo( FFlux, L, dpo, dcoef, xy_damage, LR, covidyear, abtfrac, abtlen, modes )

% modes: 0 for monte carlo + sensitivty; 1-18 for sensitivty to one parameter; 19 for sensitivty to all parameters

unc=[1	1	2	4	4	1	1	1	3	3	4	4	4	4	1	2	2	2
0.00267	1.05	53	1	1	50.3	48.5	39.1	70.71067812	447.2135955	1	1	1	1	0.4	1	1	0.2
0.000418112	0.265306122	10	0.05	0.1	5	5	5	0.433410715	0.331895407	0.1	0.1	0.1	0.5	0.2	1	1	0.1
0.000418112	0.198979592	10	0.05	0.1	5	5	5	0.433410715	0.331895407	0.1	0.1	0.1	0.5	0.2	1	1	0.2];
...
save(strcat('D:\monte carlo1\parameters-mc',num2str(mc),'.dat'),'parameters');
save(strcat('D:\monte carlo1\output_obsm-mc',num2str(mc),'.dat'),'output_obsm');
save(strcat('D:\monte carlo1\output_util-mc',num2str(mc),'.dat'),'output_util');
save(strcat('D:\monte carlo1\output_dam-mc',num2str(mc),'.dat'),'output_dam');
save(strcat('D:\monte carlo1\output_esc-mc',num2str(mc),'.dat'),'output_esc');
save(strcat('D:\monte carlo1\output_cap-mc',num2str(mc),'.dat'),'output_cap');
save(strcat('D:\monte carlo1\output_covid-mc',num2str(mc),'.dat'),'output_covid');
save(strcat('D:\monte carlo1\S8-mc',num2str(mc),'.dat'),'S8');
...
if mc==num3
    save('D:\monte carlo1\para_range.dat','para_range');
    save('D:\monte carlo1\population.dat','L');
end
